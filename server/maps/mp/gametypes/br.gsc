main()
{
    ...
    level.text_waitingPlayers = makeLocalizedString("WAITING FOR PLAYERS");
    level.text_parachuteDeployed = makeLocalizedString("PARACHUTE DEPLOYED");
    level.text_parachuteNotDeployed = makeLocalizedString("PARACHUTE NOT DEPLOYED");
    level.text_zoneIsShrinking = makeLocalizedString("ZONE SHRINKING ");
    level.text_zoneWillShrink = makeLocalizedString("ZONE SHRINKS IN ");
    ...
    level.model_parachute = "xmodel/vehicle_parachute_drop_camo";

    level.camouflages = [];
    //level.camouflages[0] = "american";
    //level.camouflages[1] = "british";
    level.camouflages[0] = "german";
    level.camouflages[1] = "russian";
...
Callback_StartGameType()
{
    ...
    game["american_model"] = mptype\american_normandy::main;
    game["british_model"] = mptype\british_normandy::main;
    game["german_model"] = mptype\german_normandy::main;
    game["russian_model"] = mptype\russian_coat::main;
    ...
    maps\mp\gametypes\_precachestuff::preCacheStrings();
...
Callback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime) //TODO: disable camera movement when in zone
{
...
Callback_PlayerKilled(eInflictor, eAttacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, timeOffset, deathAnimDuration)
{
    ...
    thread maps\mp\gametypes\_deathicons::addDeathicon(body, self.clientid, self.pers["camouflage"]);
...
checkBattleReady()
{
    ...
    level.hud_playersMin.label = makeLocalizedString("/");
        ...
        if(numberOfReadyPlayers.size > 0) //AT LEAST 1 READY PLAYER
        {
            ...
            else if(numberOfReadyPlayers.size >= level.minPlayers) //MIN PLAYERS REACHED, START COUNTDOWN
            {
                if(numberOfReadyPlayers.size <= level.maxClients && !level.startingBattle)
                {
                    ...
                    level.hud_waitingForPlayers.label = makeLocalizedString("BATTLE STARTING ");
...
startBattle()
{
    ...
    level.hud_numLivingPlayers.label = makeLocalizedString("Alive: ");
...
setupZone(zoneModeIndex)
{
    ...
    else //SHRINKING ZONE
    {
        ...
        level.zone.indexMode = zoneModeIndex;
        level.zone.life = int(level.zone.modes[zoneModeIndex]["life"]);//level.zone.life = (int)level.zone.modes[zoneModeIndex]["life"];
        level.zone.startSize = int(level.zone.modes[zoneModeIndex]["startSize"]);//level.zone.startSize = (int)level.zone.modes[zoneModeIndex]["startSize"];
        level.zone.currentSize = level.zone.startSize;
        level.zone.endSize = int(level.zone.modes[zoneModeIndex]["endSize"]);//level.zone.endSize = (int)level.zone.modes[zoneModeIndex]["endSize"];
...
keepZoneSizeVarUpdated()
{
    ...
    {
        progress = float((currentTime - startTime)) / self.life;//progress = (float)(currentTime - startTime) / self.life;
        currentSize = self.startSize + int(((self.endSize - self.startSize) * progress));//currentSize = self.startSize + (int)((self.endSize - self.startSize) * progress);
    ...
    for(;;)
    {
        if(level.zone.active)
        {
            ...
            if(inZone && !self.inZone)
            {
                ...
                self.inZone = true;
                self.hudInStormDarkness.alpha = 0;
                self.hudInStormAlert setText(makeLocalizedString(""));
            }
            else if(!inZone)
            {
                if(self.inZone)
                {
                    ...
                    self.hudInStormAlert setText(makeLocalizedString("You are in the storm!"));
                }
...
checkPlayerJumped()
{
    ...
    self.hud_jump_parachute setText(makeLocalizedString("Press ^1[{+activate}] ^7to jump"));
    ...
    for(;;)
    {
        ...
        if(self useButtonPressed() || isDefined(self.forceJump))
        {
            ...
            self.hud_jump_parachute setText(makeLocalizedString(""));
...
checkPlayerDive()
{
    ...
    self.hud_jump_parachute setText(makeLocalizedString("Press ^1[{+activate}] ^7to open/close parachute"));
...
checkVictoryRoyale()
{
    ...
    if(alivePlayers.size == 1)
    {
        ...
        winner.hud_victoryRoyale setText(makeLocalizedString("VICTORY ROYALE!"));
    ...
    else if(alivePlayers.size == 0)
    {
        ...
        level.hud_victoryRoyale setText(makeLocalizedString("NO ONE SURVIVED!"));
...
showDamageFeedback()
{
    ...
    self.hitBlip setShader("gfx/hud/damage_feedback.iwi", 24, 24);
...
loadout()
{
    ...
    case "american":
        ...
        self giveMaxAmmo("frag_grenade_american_mp");
        break;

    case "british":
        self giveWeapon("webley_mp");
        self giveMaxAmmo("webley_mp");
        self giveWeapon("frag_grenade_british_mp");
        self giveMaxAmmo("frag_grenade_british_mp");
        break;

    case "russian":
        self giveWeapon("TT30_mp");
        self giveMaxAmmo("TT30_mp");
        self giveWeapon("frag_grenade_russian_mp");
        self giveMaxAmmo("frag_grenade_russian_mp");
        break;
    ...
    case "german":
        ...
        self giveMaxAmmo("frag_grenade_german_mp");
        break;
    }
}
